...
function getMermaidStyleInstructions(): string[] {
	return [
		...
		'- Use descriptive camelCase IDs for nodes (e.g. `filterBuildings`, `onDateSelect`, `renderHeader`)',
		'- NEVER use single letters (A, B, C), generic names (node1, step2), or abbreviation prefixes (PC_, BC_)',
		'',
		'## SOURCE LINE MAPPING (REQUIRED)',
		'IMMEDIATELY after `flowchart TD`, BEFORE any subgraph or node definitions,',
		'you MUST include ALL NODE_MAP entries as mermaid comments.',
		'This maps each node ID to the source line number it corresponds to.',
		'Put them FIRST so they are never truncated.',
		'',
		'Format (one line per node):',
		'```',
		'%% NODE_MAP: nodeId -> lineNumber',
		'```',
		'',
		'Example:',
		'```',
		'flowchart TD',
		'%% NODE_MAP: computeAnalemma -> 42',
		'%% NODE_MAP: onDateSelect -> 9',
		'%% NODE_MAP: renderPoints -> 85',
		'%% NODE_MAP: handleClick -> 120',
		'',
		'subgraph ...',
		'```',
		'',
		'Rules:',
		'- Every node MUST have a NODE_MAP entry — put ALL of them right after `flowchart TD`',
